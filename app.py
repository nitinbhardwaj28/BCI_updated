...
def process_files(peak_traff_path, prov_cap_path, errors_path, availability_path):
    ...
    final_joined_df['Errors_status'] = np.select(conditions, choices, default='Yes')

    conditions1 = [
        (final_joined_df['Availability'].isnull()),
        (final_joined_df['Availability'] < 1)
    ]
    final_joined_df['Availability_status'] = np.select(conditions1, choices, default='Yes')
    ...
    final_joined_df['Download_Utl'] = np.select(conditions2, choices1, default='Bad')
    ...
    final_joined_df['upload_Utl'] = np.select(conditions2, choices2, default='Bad')
